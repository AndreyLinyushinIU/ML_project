...
class C1BilinearDeepSup(nn.Module):
    ...
    def forward(self, conv_out, segSize=None):
        ...
        if self.use_softmax:  # is True during inference
            ...
            x = torch.nn.functional.interpolate(x, mode='bilinear', size=segSize)
...
class UPerNet(nn.Module):
    ...
    def forward(self, conv_out, segSize=None):
        ...
        for pool_scale, pool_conv in zip(self.ppm_pooling, self.ppm_conv):
            ...
            #    (input_size[2], input_size[3]),
            #    mode='bilinear', align_corners=False)))
            ppm_out.append(pool_conv(torch.nn.functional.interpolate(
                pool_scale(conv5),
                (input_size[2], input_size[3]),
                mode='bilinear')))
        ppm_out = torch.cat(ppm_out, 1)
        ...
        for i in reversed(range(len(conv_out) - 1)):
            ...
            f = torch.nn.functional.interpolate(f, mode='bilinear', size=conv_x.size()[2:])
            f = conv_x + f
        ...
        for i in range(1, len(fpn_feature_list)):
            ...
            #    output_size,
            #    mode='bilinear', align_corners=False))
            fusion_list.append(torch.nn.functional.interpolate(
                fpn_feature_list[i],
                output_size,
                mode='bilinear'))
